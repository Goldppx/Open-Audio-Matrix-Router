...
<details class="card wide"><summary><b>高级矩阵</b></summary><p class=muted>勾选本机/远端设备间的格子即可创建本地或 RTP 网络线路。网络线路创建后可在路由表中管理。</p><div class=row><select id=matrixQuality><option value=low>低音质</option><option value=medium selected>中音质</option><option value=high>高音质</option></select><select id=matrixLatency><option>40</option><option>60</option><option selected>100</option><option>150</option></select><select id=matrixMode><option value=stable>稳定</option><option value=auto selected>自动</option><option value=low-latency>低延迟</option></select></div><div id=matrix class=matrix></div><button onclick=applyMatrix()>添加勾选的矩阵线路</button></details>
</div></main><script>
...
public:
    struct ActiveRoute {
        ...
        std::optional<gstreamer::NetworkAudioProfile> network_profile;
        std::function<bool(gstreamer::RtpOpusPipeline&, const gstreamer::NetworkAudioProfile&)> network_starter;
    ...
    bool add_route(std::string label, std::function<bool(gstreamer::RtpOpusPipeline&)> starter) {
        ...
        routes.push_back({next_route_id++, std::move(label), true, std::move(starter), std::nullopt, {}, std::move(pipeline)});
        return true;
    }
    bool add_network_route(std::string label, const gstreamer::NetworkAudioProfile& profile, std::function<bool(gstreamer::RtpOpusPipeline&, const gstreamer::NetworkAudioProfile&)> starter) {
        auto pipeline = std::make_unique<gstreamer::RtpOpusPipeline>(); if (!starter(*pipeline, profile)) { error = pipeline->last_error(); return false; }
        routes.push_back({next_route_id++, std::move(label), true, {}, profile, std::move(starter), std::move(pipeline)}); return true;
    }
    bool set_route_enabled(std::size_t id, bool enable) {
        for (auto& item : routes) if (item.id == id) {
            if (enable && !item.enabled) { item.pipeline = std::make_unique<gstreamer::RtpOpusPipeline>(); const bool started = item.network_profile ? item.network_starter(*item.pipeline, *item.network_profile) : item.starter(*item.pipeline); if (!started) { error = item.pipeline->last_error(); item.pipeline.reset(); return false; } }
            if (!enable && item.enabled && item.pipeline) item.pipeline->stop();
            item.enabled = enable; return true;
        }
        return false;
    }
    bool update_network_route(std::size_t id, const gstreamer::NetworkAudioProfile& profile) {
        for (auto& item : routes) if (item.id == id && item.network_profile) { item.network_profile = profile; if (!item.enabled) return true; item.pipeline = std::make_unique<gstreamer::RtpOpusPipeline>(); if (!item.network_starter(*item.pipeline, profile)) { error = item.pipeline->last_error(); item.enabled = false; return false; } return true; } return false;
    }
    ...
    bool start_remote_command(const pairing::RemoteRouteRequest& request, std::string& route_error) {
        gstreamer::NetworkAudioProfile profile; std::unordered_map<std::string, std::string> values{{"quality", request.quality}, {"mode", request.mode}, {"max-latency-ms", std::to_string(request.max_latency_ms)}};
        ...
        if (request.kind == pairing::RemoteRouteKind::Send) {
            gstreamer::SenderSettings settings; settings.host = request.host; settings.port = request.port; settings.source_device = request.device_id; settings.capture_render_device = request.render_loopback; settings.network = profile;
            if (!add_network_route("Paired send: " + request.device_id + " → " + request.host + ":" + std::to_string(request.port), profile, [settings](auto& route, const auto& next) mutable { settings.network = next; return route.start_sender(settings); })) { route_error = error; return false; }
        } else {
            gstreamer::ReceiverSettings settings; settings.port = request.port; settings.sink_device = request.device_id; settings.network = profile;
            if (!add_network_route("Paired receive: :" + std::to_string(request.port) + " → " + request.device_id, profile, [settings](auto& route, const auto& next) mutable { settings.network = next; return route.start_receiver(settings); })) { route_error = error; return false; }
        }
        return true;
    }
    ...
    std::string handle(const std::string& method, const std::string& target, std::string& type) {
        ...
        if (method == "POST" && target.rfind("/api/paired/route", 0) == 0) {
            ...
            gstreamer::NetworkAudioProfile profile; if (!network_profile_from(query, profile)) return "Invalid paired route profile.";
            ...
            pairing::RemoteRouteRequest remote; remote.device_id = remote_device->second; remote.port = media_port; remote.quality = quality_name(profile.quality); remote.max_latency_ms = profile.max_latency_ms; remote.mode = query.at("mode");
            if (kind->second == "send") {
                ...
                gstreamer::SenderSettings settings; settings.host = peer->host; settings.port = media_port; settings.source_device = local_device->second; settings.capture_render_device = loopback != query.end() && loopback->second == "true"; settings.network = profile;
                if (!add_network_route("Matrix send → " + peer->alias + ": " + remote.device_id, profile, [settings](auto& route, const auto& next) mutable { settings.network = next; return route.start_sender(settings); })) return "Local sender failed: " + error;
            } else if (kind->second == "receive") {
                gstreamer::ReceiverSettings settings; settings.port = media_port; settings.sink_device = local_device->second; settings.network = profile;
                if (!add_network_route("Matrix receive ← " + peer->alias + ": " + remote.device_id, profile, [settings](auto& route, const auto& next) mutable { settings.network = next; return route.start_receiver(settings); })) return "Local receiver failed: " + error;
        ...
        if (method == "POST" && target.rfind("/api/network/send", 0) == 0) {
            ...
            if (add_network_route("Send: " + source->second + " → " + settings.host + ":" + std::to_string(settings.port), profile, [settings](auto& route, const auto& next) mutable { settings.network = next; return route.start_sender(settings); })) { pairing.set_telemetry({quality_name(profile.quality), profile.max_latency_ms, 0.0, source->second}); pairing.announce(); return "Network sender route added and telemetry synchronized."; }
        ...
        if (method == "POST" && target.rfind("/api/network/receive", 0) == 0) {
            ...
            if (add_network_route("Receive: :" + std::to_string(settings.port) + " → " + sink->second, profile, [settings](auto& route, const auto& next) mutable { settings.network = next; return route.start_receiver(settings); })) { pairing.set_telemetry({quality_name(profile.quality), profile.max_latency_ms, 0.0, sink->second}); pairing.announce(); return "Network receiver route added and telemetry synchronized."; }
        ...
        if (method == "POST" && target.rfind("/api/routes/", 0) == 0) {
            ...
            if (target.find("/profile", slash) != std::string::npos) { gstreamer::NetworkAudioProfile profile; const auto query = query_params(target); return network_profile_from(query, profile) && update_network_route(id, profile) ? "Route properties updated." : "Could not update route properties: " + error; }
